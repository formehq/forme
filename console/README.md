...
2. **落子 session**:一次一卡,五段 v0.1 卡面(是什么 → 为什么现在 → 建议 → 拍板后会发生什么 → 落子)+ stakes 徽标 + 「你问过」问答段(#21),证据与 diff 折叠;键盘 `a/p/r` 与按钮并存;`输入修正…` 就地编辑 hunk 的 after + 一句 note(taste 原料);`问一句…`(`q` 键)发问不落子(#21,见下)。落子 toast 带本次用时(#19)。
3. **State Diff**:最新 `state-diff-*.md` 的只读投影,四段骨架原样。
4. **Metrics**(#19):time-to-decision 中位数 + 最近 20 次分布条(按 choice 着色)、重复率逐轮条(入列 vs 抑制+重复)+ 累计抑制率、世界层闸/问答往返计数。进视图时现取现算;中位数只取现场计时的落子(backfilled 不编)。
...
- **`server.ts`** — 路由:`GET /`(页面)、`GET /api/state`(投影,含 metrics)、`POST /api/presented`(卡实际上屏,静默计时起点)、`POST /api/question`(#21,发问)、`POST /api/decide`(落子;correction 事件先于 decision)。只绑 127.0.0.1 + Host 校验 + POST 强制 `application/json`(本机写路径的 CSRF 挡板)。
...
- correction 只能改 after,不能增删 hunk;park 无 un-park(与 rejected 同样被指纹永久抑制;question 是「先别落子」的出口,但 park 仍是终态)。
