...
SELECT pg_catalog.pg_advisory_xact_lock({{ACTOR_START_KEY}}::bigint);
...
SELECT CASE
  WHEN :'r4_result_http_status'::integer={{EXPECTED_HTTP}}
   ...
   AND (CASE WHEN '{{EXPECTED_EXACT_TARGET}}'='' THEN true ELSE :'r4_result_target_id'='{{EXPECTED_EXACT_TARGET}}' END)
   AND (CASE WHEN '{{EXPECTED_EXACT_VERSION}}'='' THEN true ELSE :'r4_result_target_version'='{{EXPECTED_EXACT_VERSION}}' END)
...
-- COMMIT_GATE {{ACTOR}}
SELECT CASE WHEN :'r4_commit_gate'='COMMIT {{ACTOR}}' THEN 1 ELSE pg_catalog.current_setting('forme_r4.closed_race_commit_rejected')::integer END;
COMMIT;
