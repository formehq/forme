...
| [`R4-GATE-B-PHYSICAL-ADAPTER-HOST-BINDING-CONSTRUCTION-OWNER-REVIEW.md`](./R4-GATE-B-PHYSICAL-ADAPTER-HOST-BINDING-CONSTRUCTION-OWNER-REVIEW.md) | Low-load review of the proposed exact successor Construction and its still-closed Retry/provider boundaries. |
| [`R4-GATE-B-PHYSICAL-ADAPTER-HOST-BINDING-CONSTRUCTION-PACKET.md`](./R4-GATE-B-PHYSICAL-ADAPTER-HOST-BINDING-CONSTRUCTION-PACKET.md) | Proposed exact workset and authority for fake-tested physical adapters, one post-checkpoint read-only Host Binding, and a new Yellow Retry decision surface. The requested Construction Grant is not yet approved. |
