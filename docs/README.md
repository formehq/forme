...
| [`R4-TECHNICAL-OWNER-REVIEW.md`](./R4-TECHNICAL-OWNER-REVIEW.md) | Start here for R4 technical review: ten-minute mental model, five Owner decision cards, four walkthroughs, and the boundary between Owner judgment and agent audit. |
| [`R4-TECHNICAL-CONTROL-PACKET.md`](./R4-TECHNICAL-CONTROL-PACKET.md) | Unreconciled implementation appendix: its protocol/identity/sync/lifecycle detail remains useful, but the former Vercel/Supabase and strict one-use Guest recommendations must be rewritten after the five-card Owner review. |
