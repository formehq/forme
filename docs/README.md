...
| [`R4-GATE-B-PHYSICAL-ADAPTER-HOST-BINDING-DECISION-BRIEF.md`](./R4-GATE-B-PHYSICAL-ADAPTER-HOST-BINDING-DECISION-BRIEF.md) | Proposed next direction: local-only host binding, one unified physical runner, corrected PostgreSQL race semantics, explicit acceptance that Codex post-response finality remains unproven, and a transient macOS mechanism. No new authority yet. |
